---
title: What is a TERM in SealMetrics?
slug: what-is-a-term-in-sealmetrics
sidebar_label: What is a TERM in SealMetrics?
---
[Skip to main content](#main-content)![Image](what-is-a-term-in-sealmetrics/a37799c64a3031dd8bee1ad2404decf7.png)

[Academy](https://sealmetrics.com/privacy-marketing-academy/)[Partners](https://sealmetrics.com/partners/)[Academy](https://sealmetrics.com/privacy-marketing-academy/)[Partners](https://sealmetrics.com/partners/)[All Collections](/en/)[Metric Insights](https://help.sealmetrics.com/en/collections/10013737-metric-insights)In SealMetrics,TERMrepresents thesmallest unit of measurementfor any traffic source. It helps us precisely attribute where users come from. Here’s how we define TERM across different sources:

SEO Traffic:The TERM is the landing page URL where the user arrives.Referrer Traffic:The TERM is the URL of the previous page (referrer) that sent the user.Google Ads Traffic:The TERM corresponds to the keyword that triggered the ad.UTM Term Traffic:The TERM is the value you set in the utm_term parameter.

SEO Traffic:The TERM is the landing page URL where the user arrives.

SEO Traffic:The TERM is the landing page URL where the user arrives.

Referrer Traffic:The TERM is the URL of the previous page (referrer) that sent the user.

Referrer Traffic:The TERM is the URL of the previous page (referrer) that sent the user.

Google Ads Traffic:The TERM corresponds to the keyword that triggered the ad.

Google Ads Traffic:The TERM corresponds to the keyword that triggered the ad.

UTM Term Traffic:The TERM is the value you set in the utm_term parameter.

UTM Term Traffic:The TERM is the value you set in the utm_term parameter.



This granular tracking helps you understand user intent and campaign performance better, without needing invasive tracking scripts.

[How to track Social Ads Campaigns](https://help.sealmetrics.com/en/articles/9667465-how-to-track-social-ads-campaigns)[How to track Google Ads Campaigns:](https://help.sealmetrics.com/en/articles/9667466-how-to-track-google-ads-campaigns)[How Sealmetrics Tracks Traffic Without Cookies](https://help.sealmetrics.com/en/articles/11125666-how-sealmetrics-tracks-traffic-without-cookies)[Understanding Referrer Loss and Direct Traffic in Sealmetrics](https://help.sealmetrics.com/en/articles/11125734-understanding-referrer-loss-and-direct-traffic-in-sealmetrics)[How SealMetrics Calculates SEO Traffic](https://help.sealmetrics.com/en/articles/11154410-how-sealmetrics-calculates-seo-traffic)[SealMetrics Help Center](/en/)Company

Privacy PolicyDPATerms of UseData Privacy Audit

Privacy Policy

[Privacy Policy](https://legal.sealmetrics.com/privacy-notice)DPA

[DPA](https://legal.sealmetrics.com/DPA)Terms of Use

[Terms of Use](https://sealmetrics.com/terms-of-use/)Data Privacy Audit

[Data Privacy Audit](https://app.comply.org/attest/sealmetrics)Guides

Cookie Consent GuideSocial Media AuditCookieless & Google

Cookie Consent Guide

[Cookie Consent Guide](https://sealmetrics.com/mastering-the-cookie-consent-banner/)Social Media Audit

[Social Media Audit](https://sealmetrics.com/in-depth-analysis-of-social-media-privacy-policies/)Cookieless & Google

[Cookieless & Google](https://sealmetrics.com/how-google-is-navigating-the-cookieless-future/)Explore

Check out the live demoNewsletter

Check out the live demo

[Check out the live demo](https://app.sealmetrics.com/reports/cookieless/dashboard)Newsletter

[Newsletter](https://sealmetrics.com/privacy-marketing-newsletter/)